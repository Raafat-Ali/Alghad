...
function ShowSwM(Any_Msg : String; AType: Integer = 0): Boolean;
...
procedure SetSettingValue(SName, SValue: String );
function GetSettingValue(SName: String): String;

function  ExecSQLCommand(SQLText : String) : Boolean;
...
// AType : 0-Inf 1-Success 2-ERORR  3-Warning 4-Question
 ...
 //SwMsgF.SweetAlert.confirmButtonColor:='#BDBF37';
  ...
  if UniMainModule.RTL then begin   // Arabic MSG.
   SwMsgF.SweetAlert.text :=  Any_Msg;    //  UniMainModule.gSysMessagesA.Strings[MsgINDX];
  ...
  SwMsgF.SweetAlert.text := Any_Msg;
...
+' Where TransType  =  ''' + TrnsType + '''  AND    TransNo   =     ''' + TrnsID + '''  ';
...
begin
 ...
 TempQry.SQL.Text := 'Select SetName From Settings Where SetName = ''' + SName + ''' ';
  TempQry.Open;

 if TempQry.RecordCount > 0 then begin
    SQLText := ' Update Settings  Set SetValue = ''' + SValue + ''' ' + ' Where  SetName = ''' + SName + ''' ';
 end else begin
    SQLText := ' Insert Into Settings(SetName, SetValue) ' + ' Values (''' + SName + ''', ''' + SValue + ''')';
 end;

 ExecSQLCommand(SQLText);
...
begin
 ...
 TempQry.Close;
  ...
  TempQry.SQL.Text := 'Select SetValue From Settings Where SetName = ''' + SName + ''' ';
  TempQry.Open;

  Result := TempQry.FieldByName('SetValue').AsString;
...
function  ExecSQLCommand(SQLText : String) : Boolean;
begin
  Result := UniMainModule.MainDB.ExecSQL(SQLText) = 0;
end;
//===================================================================
//===================================================================
//===================================================================
