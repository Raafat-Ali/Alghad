...
type
  TVouchersF = class(TBaseFormF)
    ...
    vouchersPayMethod: TWideStringField;
    PrintVouchQPayMethod: TWideStringField;
...
begin
  inherited;
   if  ShowSwM('Â·  —Ìœ Õ–› «·”‰œ —ﬁ„ : ' + vouchersvNo.AsString,4)then
  begin
   TempQry := TUniQuery.Create(self);
   TempQry.Connection := UniMainModule.MainDB;
   TempQry.SQL.Clear;
   TempQry.SQL.Add( 'Delete From vouchers Where vNo = ''' + vouchersvNo.AsString + '''   ');
...
//  `VID`, `vNo`, `vType`, `vSubType`, `vEXP`, `vIn`, `vOut`, `vDate` FROM `vouchers` WHERE 1
  ...
  if (NumEdtAmount.Value > 0) And (not UniEdit1.IsBlank) then begin
        ...
        vouchers.Append;
          ...
          if UniRadioButton2.Checked then begin
           ...
           vouchers.FieldByName('isMaster').AsBoolean := True;
            vouchers.FieldByName('PayMethod').AsString := PayTypeQrySection.AsString;
   ...
   ///--- Send SMS if the sub type = ⁄—»Ê‰  only.
