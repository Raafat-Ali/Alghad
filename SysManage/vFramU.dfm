object vFrame: TvFrame
  Left = 0
  Top = 0
  Width = 532
  Height = 32
  Layout = 'fit'
  LayoutAttribs.Align = 'middle'
  TabOrder = 0
  object cPa: TUniContainerPanel
    Left = 0
    Top = 1
    Width = 520
    Height = 30
    Hint = ''
    ParentColor = False
    TabOrder = 0
    Layout = 'hbox'
    LayoutConfig.Width = '90%'
    object NumEdtAmount: TUniNumberEdit
      Left = 16
      Top = 3
      Width = 121
      Height = 23
      Hint = ''
      BorderStyle = ubsInset
      Alignment = taCenter
      ParentFont = False
      Font.Color = clGreen
      Font.Height = -13
      Font.Style = [fsBold]
      TabOrder = 1
      EmptyText = '0'
      BlankValue = 0
      FieldLabel = 'Amount'
      FieldLabelWidth = 30
      FieldLabelFont.Style = [fsBold]
      DecimalSeparator = '.'
    end
    object framEdt: TUniEdit
      Left = 143
      Top = 3
      Width = 326
      Height = 23
      Hint = ''
      Text = ''
      ParentFont = False
      Font.Height = -13
      Font.Style = [fsBold]
      TabOrder = 2
      FieldLabel = #1575#1604#1576#1610#1575#1606
      FieldLabelWidth = 30
      FieldLabelFont.Style = [fsBold]
      LayoutConfig.Flex = 1
    end
    object lblKind: TUniLabel
      Left = 491
      Top = 3
      Width = 18
      Height = 13
      Hint = ''
      Visible = False
      Caption = '000'
      TabOrder = 3
    end
    object bitFree: TUniBitBtn
      Left = 472
      Top = 3
      Width = 40
      Height = 23
      Hint = ''
      Caption = '<i class="fas fa-times-circle"></i>'
      TabOrder = 4
      IconAlign = iaCenter
      LayoutConfig.Margin = '0 3 0 3'
      OnClick = bitFreeClick
    end
  end
end
